...
first_time_setup() {
    if [[ ! -f "$CCS_CONFIG_FILE" ]]; then
        ...
        echo "$SEPARATOR_LINE"
        echo "欢迎使用 Claude 配置切换器 (CCS)!"
        echo "Welcome to Claude Configuration Switcher (CCS)!"
        echo "$SEPARATOR_LINE"
        echo ""
...
show_help_zh() {
    echo -e "${BOLD}${BLUE}Claude 配置切换器 (ccs)${NC}"
...
show_help_en() {
    echo -e "${BOLD}${BLUE}Claude Configuration Switcher (ccs)${NC}"
    echo -e "${SEPARATOR_LINE}"
